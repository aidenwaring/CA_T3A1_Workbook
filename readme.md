...
```
class Car { //New car Class
  constructor(brand) {  //Constructor for car class with parameter brand
...
class Model extends Car { //Model class with Car inheritance
  constructor(brand, mod) { //Model constructor with two parameters, brand and mod
...
function randomIntFromInterval(min,max) { // min and max included //Declares a named function called randomIntFromInterval which takes two parameters, min and max
...
for (model of models) { //For loop to iterate over the models array. Model contains 40 elements.
